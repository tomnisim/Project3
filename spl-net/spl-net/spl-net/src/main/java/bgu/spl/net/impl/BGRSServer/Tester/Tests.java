        ...
        try {
            ...
            for (int i = 0; i < numThreads; i++) {
                ...
                new Thread(() -> {
                    try {
                        String username = "RON" + tempI;
                        String helper = commandsProcessors.get(tempI).sendCommand("LOGIN " + username + " " + username);
                        outputs.add(helper);
                        helper=commandsProcessors.get(tempI).sendCommand("COURSESTAT " + tempP.getCourseNum());
                        outputs.add(helper);
                        helper=(commandsProcessors.get(tempI).sendCommand("ADMINREG L" + tempI + " L" + tempI)); //Shouldn't work after you are already logged in
                        outputs.add(helper);
                        helper=(commandsProcessors.get(tempI).sendCommand("STUDENTSTAT RON" + (tempI - 1))); //ADMIN COMMAND
                        outputs.add(helper);
                        helper=(commandsProcessors.get(tempI).sendCommand("LOGOUT"));
                        outputs.add(helper);
                    } catch (Exception e) {
                        e.printStackTrace();
                    } finally {
